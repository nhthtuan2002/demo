package com.example.spingdata.restapi;

import com.example.spingdata.entity.Product;
import com.example.spingdata.service.ProductService;
...
public class ProductApi {
    ...
    public ResponseEntity<?> delete(@PathVariable int id){
        Optional<Product> product = productService.findById(id);//tim product theo id
        if (!product.isPresent()){
            ResponseEntity.badRequest().build();// khong co du lieu tra ve
        }
        productService.deleteById(id);
        return ResponseEntity.ok().build();//co du lieu tra ve
    }
}